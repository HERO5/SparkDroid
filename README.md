...
- Step2: 点击MASTER按钮以启动“工头”。（最初的做法：点击一次master之后会从"source python"复制多份相同的任务，等待Worker的任务请求，这种做法适合任务之间相互独立的场景；目前的做法：任务的区别只是其参数不同，适用于广度搜索这样的场景，详细说明见Step0中的main.py)。
